...
class Argument:
    ...
    """
    def __bool__(self):
        # Need to acount for quantities, and time expressions
        # Returns True if object has a wikidata_type and fits constraints or has no wikidata_type, otherwise returns False
        tmp = self.wikidata_types[:]
        already_popped = 0
        # create condition for counting links in the case of "number of participants" properties
        if self.wikidata_id:
            for i, link in enumerate(tmp):
                if set(link) & set(self.constraints):
                    self.type = self.dbpedia_types 
                    return True    
                elif self.wikidata_types!="None":
                    self.wikidata_types.pop(i-already_popped)
                    already_popped += 1
                    if self.type:
                        self.type.pop(i-already_popped)
            ...
            date = utils.isDate(self.text)
            if date and "TIMEX" in self.constraints:
                self.text = date
                self.type = "TIMEX"
                return True
            elif any(i.isdigit() for i in self.text.split()) and "QUANTITY" in self.constraints:
                self.text = [i.isdigit() for i in self.text.split()][0]
                self.type = "QUANTITY"
                return True
        return False
    """
...
class KG_Triple:
    def __init__(self, wikidata_class, prop, entity):
        self.head = wikidata_class
        self.relation = prop
        self.tail = entity
        pass
